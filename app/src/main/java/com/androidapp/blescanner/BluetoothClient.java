...
public interface BluetoothClient {
    /**
     * 打开蓝牙扫描操作. 如果此时正在扫描将会抛出正在扫描 { BluetoothSearchConflictException} 错误。
     * 如果想强制中断当前扫描操作，set cancel value to true.
     *
     * @param millis 扫描时间
     * @param cancel 如果正在进行扫描操作，设置是否中断当前扫描。true 中断当前扫描操作，
     *               false 如果当前正在进行扫描操作则会抛出 {BluetoothSearchConflictException} 错误
     * @return 扫描结果的列表（无重复设备）
     */
    Observable<Device> search(int millis, boolean cancel);
     ...
     * @return 成功或失败返回
     */
    Observable<String> registerNotify(String mac, UUID service, UUID characteristic,BaseResultCallback<byte[]> callback);
